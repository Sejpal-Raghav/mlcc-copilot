...
async function initModels() {
  try {
    ...
    pinnSession = await ort.InferenceSession.create(
      path.join(modelsDir, 'pinn_surrogate.onnx'),
      sessionOpts,
    );
    inspectorSession = await ort.InferenceSession.create(
      path.join(modelsDir, 'inspector.onnx'),
      sessionOpts,
    );
