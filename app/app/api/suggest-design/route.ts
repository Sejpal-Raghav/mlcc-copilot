...
function uToX(u: number[]) {
  return [
    Math.exp(
      Math.log(BOUNDS.epsilon_r.min) +
        u[0] * (Math.log(BOUNDS.epsilon_r.max) - Math.log(BOUNDS.epsilon_r.min)),
    ),
    BOUNDS.layers.min + u[1] * (BOUNDS.layers.max - BOUNDS.layers.min),
    Math.exp(
      Math.log(BOUNDS.area.min) + u[2] * (Math.log(BOUNDS.area.max) - Math.log(BOUNDS.area.min)),
    ),
    Math.exp(
      Math.log(BOUNDS.thickness.min) +
        u[3] * (Math.log(BOUNDS.thickness.max) - Math.log(BOUNDS.thickness.min)),
    ),
  ];
}
...
function xToU(x: number[]) {
  return [
    (Math.log(x[0]) - Math.log(BOUNDS.epsilon_r.min)) /
      (Math.log(BOUNDS.epsilon_r.max) - Math.log(BOUNDS.epsilon_r.min)),
    (x[1] - BOUNDS.layers.min) / (BOUNDS.layers.max - BOUNDS.layers.min),
    (Math.log(x[2]) - Math.log(BOUNDS.area.min)) /
      (Math.log(BOUNDS.area.max) - Math.log(BOUNDS.area.min)),
    (Math.log(x[3]) - Math.log(BOUNDS.thickness.min)) /
      (Math.log(BOUNDS.thickness.max) - Math.log(BOUNDS.thickness.min)),
  ];
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < NUM_SEEDS * 10; i++) {
      ...
      const epsilon_r = Math.exp(
        randRange(Math.log(BOUNDS.epsilon_r.min), Math.log(BOUNDS.epsilon_r.max)),
      );
    ...
    for (let iter = 1; iter <= MAX_ITER; iter++) {
      ...
      for (let s = 0; s < NUM_SEEDS; s++) {
        const u = seedsU[s];
        // Center point
        batchInputs.push([...uToX(u), v_bias, temperature] as [
          number,
          number,
          number,
          number,
        ...
        for (let dim = 0; dim < 4; dim++) {
          const uPlus = [...u];
          uPlus[dim] = clamp(uPlus[dim] + DELTA, 0, 1);
          const uMinus = [...u];
          uMinus[dim] = clamp(uMinus[dim] - DELTA, 0, 1);
          batchInputs.push([...uToX(uPlus), v_bias, temperature] as [
            number,
            number,
            number,
            number,
          ...
          batchInputs.push([...uToX(uMinus), v_bias, temperature] as [
            number,
            number,
            number,
            number,
    ...
    if (uniqueCandidates.length === 0) {
      return NextResponse.json(
        { error: 'Optimization failed to find any valid candidates.' },
        { status: 500 },
      );
    }
