...
export default function RootLayout({
  ...
  return (
    <html lang="en">
      <body className={`${inter.className} antialiased bg-zinc-50 text-zinc-900 min-h-screen`}>
        <header className="bg-white border-b border-zinc-200 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-14 items-center">
              <div className="flex items-center gap-2">
                <Cpu className="w-5 h-5 text-zinc-900" />
                <span className="font-semibold text-sm tracking-tight text-zinc-900 uppercase">
                  MLCC Copilot
                </span>
              </div>
              <nav className="flex space-x-1">
                <Link
                  href="/"
                  className="flex items-center gap-2 text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 px-3 py-1.5 rounded-sm text-xs font-medium transition-colors"
                >
