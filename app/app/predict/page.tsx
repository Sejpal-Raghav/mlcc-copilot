...
export default function PredictPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      area:
        parseFloat(formData.get('area_val') as string) *
        parseFloat(formData.get('area_unit') as string),
      thickness:
        parseFloat(formData.get('thickness_val') as string) *
        parseFloat(formData.get('thickness_unit') as string),
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              required
              defaultValue={1000}
              className="block w-full bg-zinc-50 border border-zinc-200 text-zinc-900 text-sm px-3 py-2 focus:outline-none focus:border-zinc-400 focus:bg-white transition-colors"
            />
          </div>
          <div>
            <label className="block text-[10px] uppercase tracking-widest font-semibold text-zinc-500 mb-1.5">
          ...
          <div>
            <label className="block text-[10px] uppercase tracking-widest text-zinc-500 mb-1 font-semibold">
              Thickness
            </label>
            <div className="flex gap-2">
              <input
          ...
          <div className="pt-2 flex gap-3">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 flex justify-center py-2.5 px-4 border border-zinc-900 text-xs font-semibold text-white bg-zinc-900 hover:bg-zinc-800 focus:outline-none disabled:opacity-50 transition-colors uppercase tracking-wider"
            >
            ...
            {history.length > 0 && (
              <button
                ...
                onClick={handleClear}
                disabled={loading}
                className="flex justify-center py-2.5 px-4 border border-zinc-200 text-xs font-semibold text-zinc-900 bg-white hover:bg-zinc-50 focus:outline-none disabled:opacity-50 transition-colors uppercase tracking-wider"
              >
                Clear
              </button>
            )}
        ...
        {currentResult ? (
          <div className="bg-white border border-zinc-200 p-6 h-full flex flex-col">
            <div className="flex items-center justify-between mb-6 border-b border-zinc-100 pb-4">
              <h3 className="text-[10px] uppercase tracking-widest font-semibold text-zinc-500">
                Inference Output {history.length === 2 && '(Comparison Mode)'}
              </h3>
            </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={mergedCurve} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                    ...
                    <Tooltip
                      ...
                      formatter={(val: any) => [
                        val ? Number(val).toExponential(4) + ' Ω' : '',
                        'Impedance',
                      ]}
                    />
                    <Line
                      type="monotone"
                      dataKey={history.length === 2 ? 'Baseline' : 'z'}
                      stroke="#18181b"
                      strokeWidth={2}
                      dot={false}
            ...
            <div className="mt-6 bg-zinc-50 border border-zinc-200 p-4">
              ...
              <ul className="text-xs text-zinc-600 leading-relaxed space-y-3">
                <li>
                  <strong className="text-zinc-800">Why the V-Shaped Curve?</strong> The graph shows
                  how impedance changes with frequency. At low frequencies, it acts as a capacitor
                  (impedance drops). The minimum point is the <em>Resonant Frequency</em>, where
                  parasitic inductance (ESL) cancels capacitance. At higher frequencies, the ESL
                  dominates and it acts like an inductor (impedance rises).
                </li>
                <li>
                  <strong className="text-zinc-800">Physical Derating:</strong> Try increasing the
                  DC Bias or changing the Temperature. The PINN has learned real-world physics:
                  high-K ceramics lose capacitance under high electric fields (voltage derating) and
                  drift with temperature. A traditional formula would miss this entirely.
                </li>
                <li>
                  <strong className="text-zinc-800">Under the Hood:</strong> The model combines a
                  hard-coded <em>Physics Branch</em> (computing ideal{' '}
                  <code className="bg-zinc-200/50 px-1.5 py-0.5 rounded font-mono text-[10px] text-zinc-800 tracking-wider">
                    C = ε₀ · εᵣ · A · N / d
                  </code>
                  ) with a neural network <em>Residual Branch</em> that predicts the non-ideal
                  deviations based on 50,000 simulated physics samples.
                </li>
              </ul>
            </div>
